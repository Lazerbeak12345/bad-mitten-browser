...
(define (html-br? theXexp)
  (and (xexp? theXexp) (not (string? theXexp)) (eq? 'br (xexp-name theXexp))))
...
(define (xexp->dom xexp #:parent parent #:editor editor #:doctype [doctype 'quirks])
  ...
  (for ([elm xexp])
    (cond
      [(string? elm) (set! last-string (string-append last-string elm))]
      [(xexp-short? elm)
       (set! last-string (string-append last-string (string (xexp-short->char elm))))]
      [else
       (append/last-string!)
  ...
  (for/list ([elm cleaned-elms]
             #:when (let ([d (xexp-decl? elm)])
                      (when d
                        (set! doctype (get-decl-doctype elm)))
                      (not d)))
    (cond ; TODO handle style and script tag content
      [(string? elm) (make-object string-snip% elm)]
      [(or (xexp-with-attrs? elm) (xexp-no-attrs? elm))
       (new
        dom-elm%
        [name (xexp-name elm)]
        ...
        [children
         (lambda (child-parent)
           (xexp->dom (xexp-children elm) #:parent child-parent #:editor editor #:doctype doctype))])]
      [else (error 'xexp->dom "You've disloged a forign object in my parse expander!")])))
