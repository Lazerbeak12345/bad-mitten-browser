...
|#
(struct box-bounding ([x : Real] [y : Real] [w : Real] [h : Real]))
(provide box-bounding
         box-bounding?
         box-bounding-x
         box-bounding-y
         box-bounding-w
...
(define (add-box-boundings left right)
  ...
  (box-bounding (box-bounding-x left) (box-bounding-y left) left-w left-h))
...
(provide box-bounding-right
         box-bounding-bottom
         box-bounding-too-right?)

(struct location ([x : Real] [y : Real]))
(provide location
         location?
         location-x
         location-y)
...
(define (location-return-left cursor min-size)
  (location (box-bounding-x min-size) (location-y cursor)))
...
(define (location-new-line cursor occupied h)
  (location (location-x cursor) ((box-bounding-y occupied) . + . (max (box-bounding-h occupied) h))))
...
(define (location-nl/cr cursor occupied h min-size)
  (location-return-left (location-new-line cursor occupied h) min-size))
