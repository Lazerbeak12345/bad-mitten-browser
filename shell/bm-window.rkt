...
(require (only-in racket/math exact-ceiling)
         (only-in typed/images/icons back-icon metal-icon-color play-icon text-icon x-icon)
         (only-in typed/net/url netscape/string->url url? url->string URL)
         ;typed/pict
         (only-in typed/racket/class class define/private init new object% send super-new)
         (only-in typed/racket/gui/base
                  ...
                  Text-Field%
                  Control-Event%)
         (only-in "custom-tab-panel.rkt" tab-panel-closable% Tab-Panel-Closable%)
...
(define (char->icon char)
  (text-icon char
             ...
             #:trim? #t))
(define-type Bm-window% (Class (init [links (U Null String (Listof String) URL (Listof URL))])))
  ...
  (class object%
    ...
    (define self-links
      ...
      (let ([_links links]) ; Only initialize links once
        (cond
          [(null? _links) (list (netscape/string->url "bm:newtab"))]
          ...
          [(url? _links) (list _links)]
          [(string? _links) (list (netscape/string->url _links))])))
    (define label
      :
      String
      "Bad-Mitten Browser")
    (define frame
      ...
      (let-values ([(width height) (get-display-size)])
        ...
        (unless scaleWindow
          (log-info "Scaling initial window size!"))
        (new frame%
             [label label]
             ...
             [width (if (width . and . scaleWindow) (exact-ceiling (width . * . .585651537)) 800)]
             [height (if (height . and . scaleWindow) (exact-ceiling (height . * . .78125)) 600)]
    ...
    (: locationChanged : (Instance Text-Field%) (Instance Control-Event%) -> Void)
    ...
    (define addTabBtn
      ...
      (new button%
           ...
           ;[label "New Tab"]
           [label (char->icon "+")]
           [callback (lambda (button event) (addTabBtnCallback))]))
    (define locationBack
      :
      (Instance Button%)
      (new button%
           ...
           [callback (lambda (button event) (send (getCurrentTab) back))]))
    (define locationForward
      ...
      (new button%
           ...
           [callback (lambda (button event) (send (getCurrentTab) forward))]))
    (define locationReload
      ...
      (new button%
           ...
           ;[label "Reload"]
           [label (char->icon "⟳")]
           [callback (lambda (button event) (send (getCurrentTab) reload))]))
    ...
    (define tab-elm
      :
      (Instance Tab-Panel-Closable%)
      (new tab-panel-closable%
           [choices
            (for/list ([link self-links])
              (url->string link))]
    ...
    (define/private (addTabBtnCallback)
      ...
      (when ((length tabs) . = . 1)
        (hideTabRow #f))
      (set! tabs (append tabs (list (makeTab (netscape/string->url "bm:newtab")))))
    ...
    (define/private (closeGivenTab index)
      ...
      (send tab-elm
            ...
            (cast (clamp 0
                         ((length tabs) . - . 1)
                         (if focused-left-of-close (current-focused . - . 1) current-focused))
                  Integer))
      (when ((length tabs) . = . 1)
        (hideTabRow #t))
      (do-focus))
    ...
    (define/private (do-focus)
      (let ([index
             :
             Integer
             ((send tab-elm get-selection) . or . 0)])
        (send (list-ref tabs index) focus)
        (set! last-tab-focused index))
      (update-title))
    (: set-title : String -> Void)
    (define/private (set-title title) (send frame set-label (format "~a - ~a" title label)))
    (: update-title : -> Void)
    (define (update-title)
