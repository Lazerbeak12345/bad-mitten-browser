...
(define tab%
  ...
  (class object%
    ...
    (define/private (initRenderer)
      (log-info "Starting renderer on ~a" (url->string self-url))
      (clean)
    ...
    (define/private (navigate-to the-url)
      (log-info "Navigating to '~a'" title)
    ...
    (super-new)
    (define/public (close)
      (log-info "Closing ~a" (url->string self-url))
    ...
    (define/public (locationChanged)
      ...
      (if (equal? self-url new-url)
          ...
          (let ([self-url-string (url->string self-url)] [new-url-string (url->string new-url)])
            (log-info "Changing '~a' to '~a'" self-url-string new-url-string)
            (send ext-locationBox set-value new-url-string)
            (set! history (cons self-url history))
            (set! history-future '())
            (navigate-to new-url))))
    (define/public (focus)
      (log-info "Focusing '~a'" (url->string self-url))
      (when (null? renderer)
        (initRenderer))
    ...
    (define/public (unfocus)
      (log-info "Unfocusing '~a'" (url->string self-url))
      (send ext-tab-holder delete-child thisPanel))
    (define/public (reload)
      (log-info "Reloading '~a'" (url->string self-url))
      (navigate-to self-url))
    (define/public (back)
      (log-info "Going back on '~a'" (url->string self-url))
    ...
    (define/public (forward)
      (log-info "Going forward on '~a'" (url->string self-url))
