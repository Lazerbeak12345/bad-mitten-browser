...
(require (only-in racket/list first)
         (only-in typed/net/url path/param-path url->string url-host url-path Path/Param URL)
...
(define (makeErrorMessage e)
  `(*TOP* (*DECL* DOCTYPE html)
          (html (body (@ (style "height:100%")) (strong (@ (style "margin:auto;")) ,e)))))
...
(define (directory-page path-string theUrl/path show-hidden)
  ...
  `(*TOP*
    (*DECL* DOCTYPE html)
    (html (body (h1 ,(format "File Listing of ~a" path-string))
                (ul . ,(for/list :
                         (Listof Xexp)
                         ([name dir-list])
                         (let ([name/str (bytes->string/locale (path->bytes name))])
                           (if (((string-ref name/str 0) . eq? . #\.) . and . (not show-hidden))
                               ""
                               (let ([full/location (bytes->string/locale
...
(define (bmUrl theUrl)
  ...
  (define paths
    (let ([paths-before (for/list :
                          String/Up/Same
                          [(path (url-path theUrl))]
                          (path/param-path path))]
          [host (url-host theUrl)])
      (if (not host) paths-before (cons host paths-before))))
  (case (if (null? paths) "newtab" (first paths))
    [("about" "urls" "bm")
     `(*TOP*
       (*DECL* DOCTYPE html)
       (html (head (title "Bad Mitten URLS"))
             (body (h1 "Bad Mitten" (i "Browser"))
                   (ul . ,(for/list :
                            (Listof Xexp)
                            ([theUrl : String '("bm:about" "bm:blank" "bm:bm" "bm:newtab" "bm:urls")])
    ...
    [("newtab")
     '(*TOP* (*DECL* DOCTYPE html)
             (html (head (title "New Tab"))
                   (body (h1 "Bad Mitten" (i "Browser"))
                         (span (@ (style "font-size:.5em; color:grey"))
                               "See the"
                               (& nbsp)
                               (a (@ (href "bm:urls")) "built-in urls")))))]
    [else (makeErrorMessage (format "Page does not exist '~a'" (url->string theUrl)))]))
